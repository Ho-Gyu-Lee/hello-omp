...
설치본에서 확인한 빌트인 에이전트는 `scout`·`reviewer`·`security-reviewer`·`task`·`sonic` 5종이다. 기본 원칙은 제공되는 빌트인을 그대로 사용하는 것이다 — 같은 이름으로 복제·오버라이드하지 않는다(번들 프롬프트 stale·빌트인 컨텍스트 상속 손실 방지).
